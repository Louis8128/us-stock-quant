...
DEFAULT_CACHE_DIR = SCRIPT_DIR / "data_cache" / "us_stock"
...
def resample_month_end_last(data: pd.DataFrame | pd.Series) -> pd.DataFrame | pd.Series:
    """
    Select the last available trading observation of each calendar month.

    This keeps the index as the actual last trading date, not the calendar
    month-end label. That matters because US month-end can fall on a weekend or
    holiday; using a calendar label would later fail to align with daily factor
    values and silently drop valid months.
    """
    if data.empty:
        return data.copy()
    ...
    month_end_dates = (
        sorted_data.index.to_series()
        .groupby(sorted_data.index.to_period("M"))
        .max()
    )
    return sorted_data.loc[pd.DatetimeIndex(month_end_dates)]
...
class DataDownloadConfig:
    ...
    cache_dir: str | Path = field(default_factory=lambda: DEFAULT_CACHE_DIR)
    use_cache: bool = True
...
def run_residual_volatility_backtest(
    ...
    print(f"月度有效样本数: {len(common_month_end_dates)}")
